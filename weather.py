...
data = pd.read_csv("weatherAUS.csv")[:2000]
...
y = np.array(train_data["RainTomorrow"])
x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.2)
x_test_out = np.array(test_data.drop(["RainTomorrow"], axis=1))
y_test_out = np.array(test_data["RainTomorrow"])
